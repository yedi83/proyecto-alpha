...
# -*- coding: utf-8 -*-
"""
exp-002 — ¿La cesta tolera riesgo por símbolo (BTC 0.15%, resto 0.10%)?
...
Umbral de decisión (escrito ANTES de correr):
  ACEPTA si, en FULL: Sharpe(btc015) >= Sharpe(base) - 0.05
                  y  Calmar(btc015) >= Calmar(base) * 0.85
...
RISK_BTC015 = {**{s: 0.001 for s in BASKET}, "BTCUSDT": 0.0015}  # decisión M1
...
def main():
    ...
    for wname,(a,b) in WINDOWS.items():
        ...
        for cfg, rmap in (("base", RISK_BASE), ("btc015", RISK_BTC015)):
    ...
    fb_, fv_ = res["full/base"], res["full/btc015"]
    dd_ok = all(res[f"{w}/btc015"]["maxdd"] >= res[f"{w}/base"]["maxdd"] - 0.05 for w in WINDOWS)
    acepta = (fv_["sharpe"] >= fb_["sharpe"] - 0.05) and (fv_["calmar"] >= fb_["calmar"]*0.85) and dd_ok
    ver = "ACEPTA" if acepta else "RECHAZA"
    L=[f"# exp-002 — riesgo por símbolo (BTC 0.15%) — {datetime.now(timezone.utc):%Y-%m-%d %H:%M} UTC\n",
