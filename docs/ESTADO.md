...
> Única fuente de verdad del estado. Última actualización: 2026-07-19.

## Fase actual

Reestructuración del proyecto sobre esta base documental + validación operativa de H-001 (Canal de Donchian).

**Banco C-001: ABIERTO (2026-07-19), F0 cerrada.** F0 pasó re-arbitraje A-04 **CONFORME** (`F0_DICTAMEN_A04_v2.md`) tras ratificar ADR-0008 (mandato de modelo → "mejor disponible a criterio del IP" + ratificación de la F0 por Fable 5) y ADR-0009 (resiliencia = compuerta). Acta sellada. Falta solo el `git tag banco-C001-abierto` (acto del IP). **F1 CERRADA + F2 EJECUTADA 2026-07-20.** F1: mapeo PRISMA, 30 entradas 100% verificadas, A-04 CONFORME + aprobación IP. F2 **CERRADA** (árbol genealógico, 30 entradas en 5 mecanismos —M1 Persistencia I-II candidato, H-001 hereda; M2 Carry, M3 Reversión, M4 Microestructura, M5 Régimen-filtro— con herencia de evidencia y 3 casos frontera; JSONL `fases/F2_arbol.jsonl` + vista `F2_ARBOL.md`; A-04 CONFORME v3 tras 2 remediaciones + aprobación IP). Pendientes anotados para C-002: O-1 (subfamilia) y C-001 (contradicción del ORQUESTADOR sobre el JSONL de F2). **F3 CERRADA 2026-07-21** (evidencia + auditoría de robustez, 30 edges; `fases/F3_evidencia.jsonl` + `F3_EVIDENCIA.md`; resumen legible en `BANCO/RESUMEN_ESTRATEGIAS.md`; A-04 CONFORME v2 tras 1 remediación + aprobación IP). Hallazgo: 36 pruebas de robustez reportadas / 127 no (walk-forward y bootstrap 0/30). Pendiente para C-002: C-001 (niveles I-VII vs entradas método/teoría). **F4 CERRADA 2026-07-21** (fundamento económico + falsabilidad: 10 fundamento sólido —persistencia/H-001, carry, reversión de largo plazo—, 9 degradados, 1 descartado, 10 no-proceden; correcciones de asignación M3/ILLIQ/VPIN; A-04 CONFORME + aprobación IP; C-001 resuelta retirando 2 frases comparativas; `fases/F4_economia.jsonl` + `F4_ECONOMIA.md`). **F5 CERRADA 2026-07-23** (transferibilidad: 8 viaja —persistencia/H-001 + carry—, 2 con condiciones —arb funding, ILLIQ—, 9 no-viaja —reversión y microestructura—, 11 n/a; A-04 CONFORME v2 tras 1 remediación —`DATA.md` actualizado a exp-008 + claim de funding afinado— + aprobación IP; `fases/F5_transferencia.jsonl` + `F5_TRANSFERENCIA.md`). **F6 CERRADA 2026-07-23** (diseño experimental, doble pasada 6a diseñador + 6b adversarial; protocolo principal = CARRY delta-neutral spot/perp; 16 amenazas incorporadas —11 pruebas + 5 limitaciones—; A-04 CONFORME verificando los 10 aspectos del IP + aprobación IP; validación limpia **bloqueada por Data Lake** —survivorship de la cola del carry-crash, spot real, capacidad—; `fases/F6_PROTOCOLOS.md` + `F6_protocolos.jsonl`). **F7 (priorización + mapa de complementariedad + top-N a la cola) DESBLOQUEADA — última fase del ciclo.** El Data Lake emerge como cuello de botella de infraestructura (mismo que bloquea ILLIQ-MR-001). Banco ≤30% (moratoria ADR-0004). **Cola post-C-001 / prioridad Data Lake:** candidata `ILLIQ-MR-001` (H-003, reversión de corto plazo condicional a la iliquidez; null por defecto = artefacto, compuerta de cosechabilidad neta) registrada 2026-07-23 en `BANCO/COLA_CANDIDATAS/` — **no ejecutar**; formalizar en C-002 solo tras definir universo estratificado + PREREG. Prioridad sigue siendo evidencia (H-002.v2 + Fase B de H-001).
...
|---|---|---|---|
| [H-001](INVESTIGACION/hipotesis/H-001-canal-donchian.md) | Canal de Donchian 512 (perp USDT) | Fase A APROBADA 6/6 (07-14). Fase B: corte del 07-14 **COMPROMETIDO** (contaminación: ensayo con keys idénticas operó la misma cuenta demo → doble bot). **RE-CORTE limpio 2026-07-16, corte 15:27:33 UTC** (`fase_B/ACTA_RECORTE_2026-07-16.md`): keys del ensayo neutralizadas, cuenta aplanada, tarea `faseB_donchian` (python directo, IgnoreNew), datos 07-14→16 anulados, reloj de 1 mes reiniciado. Corriendo: demo, eq=750, abiertas=0. **Estatus epistemológico: CUESTIONADA (ADR-0006, 07-17)** — exp-004 disparó letal de lookback y el override por nota se formalizó retroactivamente; **exp-008 CUMPLIDO EN R0 (07-17):** funding real mejora todas las ventanas (dataset APTO A-02, hashes congelados, paridad OK) → la mayor incógnita declarada de H-001 se cierra A FAVOR, **sujeto a ratificación del ADR-0006** (Propuesta v2 tras dictamen A-04 NO CONFORME del 07-18: 6 violaciones verificadas y remediadas — ficha al día, exp-005 cubierta, enmienda constitucional propuesta; pendiente re-arbitraje + ratificación del IP) | ⏳ En validación — **Fase B corriendo · ADR-0007 RATIFICADO (07-18: CONFORME al 3er arbitraje → principio 17 en la Constitución, retroactividad prohibida para siempre, Fase A convalidada, condición (i) vigente, cláusula de 2º disparo sobre Fase B). Estatus y condiciones: ver ficha (fuente única)** |
| [H-002.v1](INVESTIGACION/hipotesis/H-002-ruptura-bnb-d1.md) | Ruptura Donchian(20)+SMA200, diario, **solo largos** (majors) | **RECHAZADA 2026-07-19 — disparo T1 firme (principio 17).** El banco del 07-14 se apoyaba en la anulación interpretativa de un veredicto letal pre-escrito (T1 "el decisivo" disparó MATA y fue anulado en el mismo `RESULTADO`, sin ADR/A-04/ratificación; transitoria cerrada, sin rescate posible). Ninguna evidencia con dictamen A-01. Alta tardía en el registro maestro subsanada el mismo día. | ❌ RECHAZADA (`PROTOCOLO §5` → versionado) |
| [H-002.v2](INVESTIGACION/hipotesis/H-002.v2-ruptura-bnb-d1.md) | Ruptura Donchian(20)+SMA200, diario, solo largos (v2) | Ordenada por el IP el 07-19. PREREG v2 con **umbrales fijados y explícitos** (`research/H-002-ruptura-bnb-d1/v2/PREREG_BANCO.md`): decisivo = **cesta multi-activo, screen pre-2023 ∧ forward OOS** (risk-matched eliminado por invariancia de escala); funding en la corrida decisiva; **A-01 obligatorio**; lockbox 2023-26 **quemado** (excluido del kill); horizonte forward mecánico; §8 justificación. Señal idéntica a v1. **PREREG SELLADO 2026-07-19 (cero parámetros abiertos); no corrido, pendiente del push.** Cláusula de "evidencia insuficiente" mientras el forward no cumpla ≥30 trades ∧ ≥6 meses. | ⏳ EN VALIDACIÓN — pre-registro sellado, no corrido |
